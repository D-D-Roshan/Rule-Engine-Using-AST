...
exports.evaluateRule = async (req, res) => {
  try {
    ...
    console.log('Incoming request:', { ast, data });

    // Fetch the rule from the database
    const rule = await Rule.findOne({ ruleName: ast }); // Use findOne to get a single rule
    ...
    // Call the evaluate function with the rule AST and the incoming data
